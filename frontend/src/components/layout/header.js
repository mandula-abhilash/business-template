    ...
    <>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[110]">
          ...
          {/* Menu panel */}
          <div
            className="fixed inset-y-0 right-0 w-full bg-background sm:max-w-sm sm:ring-1 sm:ring-border/10 flex flex-col"
            role="dialog"
            aria-modal="true"
          >
            {/* Mobile menu header */}
              ...
              <div className="flex flex-col gap-4">
                {navigationLinks.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className={`rounded-lg px-3 py-2 text-base font-semibold leading-7 ${
                      pathname === item.href
